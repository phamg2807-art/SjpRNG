...
def get_rarity_settings(chance):
    ...
    if chance >= 1000000000: return discord.Color.from_str("#000000"), True, True
    if chance >= 100000000: return discord.Color.from_str("#FF69B4"), True, False
    if chance >= 10000000: return discord.Color.blue(), False, False
    if chance >= 1000000: return discord.Color.from_str("#FFC0CB"), False, False
    if chance >= 100000: return discord.Color.teal(), False, False
    if chance >= 10000: return discord.Color.orange(), False, False
...
@bot.event
async def on_ready():
    await bot.tree.sync() # CRITICAL: This syncs the commands to Discord
...
async def on_message(message):
    ...
    for prize in prizes:
        if random.randint(1, prize['chance']) == 1:
            ...
            target_channel = bot.get_channel(ANNOUNCE_CHANNEL_ID) if special else message.channel
